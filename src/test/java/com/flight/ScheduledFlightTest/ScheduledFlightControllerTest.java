...
class ScheduledFlightControllerTest {
	...
	void testDeleteScheduledFlight() {
		...
		userController.deleteScheduledFlight(new BigInteger("7"));
		if(sfrRepo.findById(new BigInteger("7")).isPresent())
		{
			checked=true;
		}
	...
	void testModifyScheduleFlight() {
		ScheduledFlight sf=sfrRepo.findById(new BigInteger("7")).get();
